...
class TensorboardCallback(BaseCallback):
    ...
    def _on_rollout_end(self):
        ...
        # run sim (to plot traj)
        done = False
        state = env.reset()
        actions = []
        while not done:
            action, _ = self.model.predict(state, deterministic=True)
            state, reward, done, info = env.step(action, normalize_action=True)
        ...
        ax.set_xlabel('Total males (log scale)')
        ax.set_ylabel('Total females (log scale)')
        ax.set_xscale('log')
        ax.set_yscale('log')
        ...
        plt.tight_layout()
        self.logger.record(f'eval/heatmap', Figure(fig, close=True),
        ...
        # ax.legend()
        plt.tight_layout()
        self.logger.record(f'eval/traj', Figure(fig, close=True),
                            exclude=('stdout', 'log', 'json', 'csv'))
        plt.close(fig)
