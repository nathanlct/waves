...
if False:
    ...
    for k in range(2):
        ...
        ax.scatter(total_males[0], total_females[0], s=30, marker="o", color="r", zorder=100)
        ax.scatter(total_males[-1], total_females[-1], s=50, marker="x", color="r", zorder=100)

        x_total_males = [np.linspace, np.geomspace][k](*np.maximum(1e-5, ax.get_xlim()), 500)
        y_total_females = [np.linspace, np.geomspace][k](*np.maximum(1e-5, ax.get_ylim()), 500)
    ...
    for k in range(2):
        ...
        if k == 1:
            ax.set_yscale("log")
        ax.legend(bbox_to_anchor=(0.0, 1.02, 1.0, 0.102), loc="lower left", mode="expand", borderaxespad=0.0)
